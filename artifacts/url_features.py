...
def _brand_lookalike(host: str, path: str) -> Dict[str, int]:
    """
    Detect simple brand lookalikes using a leet-speak normalization.
    E.g. go0gle -> google, paypa1 -> paypal, faceb00k -> facebook.
    """
    brands = ["google","apple","microsoft","amazon","paypal","facebook","instagram","netflix","bank","meta"]
    ...
    # Normalize second-level domain and subdomain separately
    host_parts = _split_host(h)
    sld = host_parts["sld"] or ""
    sub = host_parts["sub"] or ""

    sld_norm = sld.translate(leet_map)
    sub_norm = sub.translate(leet_map)
    path_norm = p.translate(leet_map)

    return {
        "brand_lookalike_domain": int(any(b in sld_norm for b in brands)),
        "brand_lookalike_subdomain": int(any(b in sub_norm for b in brands)),
        "brand_lookalike_path": int(any(b in path_norm for b in brands)),
    }
...
class URLFeatureizer(BaseEstimator, TransformerMixin):
    ...
    def _extract_row(self, url_text: str) -> Dict[str, Any]:
        ...
        feats.update(_brand_hits(host, path))
         feats.update(_brand_lookalike(host, path))
